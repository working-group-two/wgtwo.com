...
module.exports = (_, { 
  ...
  async contentLoaded({content, actions: { addRoute, createData }}) {
    const dom = new JSDOM(content.replaceAll("<![CDATA[", "").replaceAll("]]>", ""))
    const jobsData = Array.from(dom.window.document.querySelectorAll("position")).map(p => ({
