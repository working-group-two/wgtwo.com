...
import { openTwitterPopup, openRedditPopup, openLinkedinPopup } from "../../../../util/social-share-behavior";
  ...
  return (
    <div className={clsx(styles.container, 'margin-vert--md', className)}>
      ...
      <div className={styles.iconsContainer}>
        <button className={styles.button} onClick={() => onLinkedinClick(baseUrl, metadata)}>
          <LinkedinIcon className={styles.icon} />
        </button>
        <button className={styles.button} onClick={() => onTwitterClick(baseUrl, metadata)}>
          <TwitterIcon className={styles.icon} />
        </button>
        <button className={styles.button} onClick={() => onRedditClick(baseUrl, metadata)}>
          <RedditIcon className={styles.icon} />
        </button>
        <span className={styles.spacer}>|</span>
        <button className={styles.button} onClick={() => onCopyLink(baseUrl, metadata)}>
          <Link className={`${styles.icon} ${styles.iconLink}`} strokeWidth={2.5} />
        </button>
      </div>
    </div>
  );
